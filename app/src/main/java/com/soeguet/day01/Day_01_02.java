...
public class Day_01_02 {
    ...
    public Day_01_02() {
        ...
        Stream<String> lines = init.lines();
        ...
        lines.forEach(line -> {
            ...
            MyLoop: for (int i = 0; i < characterArray.length; i++) {

                AtomicInteger atomicInteger = new AtomicInteger(i);

                if (Character.isDigit(characterArray[i])) {

                    numbers.add(Character.getNumericValue(characterArray[i]));
                    continue MyLoop;
                }

                dict.forEach((k, v) -> {

                    if (atomicInteger.get() + k.length() <= line.length()
                            && k.startsWith(Character.toString(characterArray[atomicInteger.get()]))) {

                        String checkString = line.substring(atomicInteger.get(), atomicInteger.get() + k.length());

                        if (checkString.contains(k)) {
            ...
            String first = numbers.getFirst().toString();
            String last = numbers.getLast().toString();

            result.getAndAdd(Integer.parseInt(first + last));
        });
