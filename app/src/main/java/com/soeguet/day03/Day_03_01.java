...
public class Day_03_01 {
        ...
        for (int row = 0; row < matrix.length; row++) {
            ...
            for (int column = 0; column < matrix[0].length; column++) {
                ...
                if (Character.isDigit(x)) {
                    ...
                    int valueAtPoint = grabValueAtPoint(matrix, row, column);
                    System.out.println("Value at Point: " + valueAtPoint);

                    // skip ahead according to digit count of int
                    int skipCounter = String.valueOf(valueAtPoint).length() - 1;

                    column += skipCounter;

                    System.out.println("was nun hinzugefuegt wird ist: " + valueAtPoint);
                    System.out.println("____");
                    System.out.println("");

                    valuesList.add(valueAtPoint);
                }
            }
        }

        int abc = valuesList.stream().mapToInt(Integer::intValue).sum();
        System.out.println("SUM AT THE END: " + abc);
    }
    ...
    private int grabValueAtPoint(char[][] matrix, int row, int column) {
        ...
        do {
            ...
            if (!Character.isDigit(spot)) {
                ...
                if (isAddable) {
                    ...
                    isAddable = checkIfAddable(matrix, row, currentColumn);
                }
        ...
        System.out.println(valueAtPoint);

        return isAddable ? Integer.parseInt(valueAtPoint) : 0;
    }

    private boolean checkIfAddable(char[][] matrix, int row, int column) {
