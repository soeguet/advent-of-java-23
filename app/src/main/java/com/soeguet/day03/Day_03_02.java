...
public class Day_03_02 {
        ...
        for (int r = 0; r < rowArray.length; r++) {
            ...
            for (int c = 0; c < columnArray.length; c++) {
                ...
                if (Character.isDigit(x)) {
                    ...
                    GrabResultRecord grabResultRecord = grabValueAtPoint(matrix, currentRow, currentColumn);
                    // TODO need to handle skip -> if value extends to the right, all is ok. if
                    // if extends to the left, I need to handle that
                    int skipCounter = grabResultRecord.skipForward();
                    c += skipCounter;
                    neighbourDigitList.add(grabResultRecord.value());
                }
            }
        }
    ...
    /**
     * return the value and the number of spots to skip forward to
     * 
     * @param value
     * @param skipForward
     */
    record GrabResultRecord(int value, int skipForward) {
    }

    private GrabResultRecord grabValueAtPoint(char[][] matrix, int row, int column) {

        String valueAtPoint = "" + matrix[row][column];
        int jumpForwardCounter = 0;
        ...
        return new GrabResultRecord(Integer.parseInt(valueAtPoint), jumpForwardCounter);
    }
